...
import { Menu, Info, Loader2, Monitor, Settings, PanelLeftClose, PanelLeftOpen } from 'lucide-react';
...
export default function App() {
  ...
  const handleSubmit = async () => {
    ...
    await updateChat(currentChat.id, { messages: messagesWithAssistant });
    ...
    try {
      ...
      await streamMessage(messagesForModel, (chunkText) => {
        ...
        setChats(prev => prev.map(c => c.id === currentChat.id ? { ...c, messages: updatedMsgs, updatedAt: Date.now() } : c));
      });

      const finalMsgs = [...newMessages, { ...assistantMsg, content: fullText }];
      await updateChat(currentChat.id, { messages: finalMsgs });
    } catch (err: any) {
      console.error(err);
      const updatedMsgs = [...newMessages];
      updatedMsgs.push({ ...assistantMsg, content: err.message || 'Error: Generation failed.' });
      await updateChat(currentChat.id, { messages: updatedMsgs });
    } finally {
  ...
  return (
    <div className="flex h-screen bg-background text-foreground overflow-hidden">
      ...
      <main className="flex-1 flex flex-col relative h-full max-w-full">
        {/* Header */}
        <header className="h-14 border-b border-border flex items-center px-4 justify-between bg-card/50 backdrop-blur-sm z-10 shrink-0">
            ...
            {currentChat?.messages.length === 0 ? (
              ...
              currentChat?.messages.map(msg => (
                <div key={msg.id} className={`flex gap-4 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {msg.role !== 'user' && (
                    <div className="w-8 h-8 shrink-0 rounded-full flex items-center justify-center bg-white text-black border border-gray-200">
                      <Logo size={18} />
                    </div>
                  )}
                  <div className={`min-w-0 max-w-[85%] ${msg.role === 'user' ? 'bg-muted px-5 py-3 rounded-3xl' : ''}`}>
        ...
        <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-background via-background to-transparent pt-6 px-4 md:px-8">
